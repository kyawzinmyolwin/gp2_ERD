...
DROP FUNCTION IF EXISTS fn_sync_line_sponsorship_donor_profile_id() CASCADE;
DROP FUNCTION IF EXISTS fn_enforce_group_area_min_vertices() CASCADE;
...
CREATE TABLE line (
    ...
    CONSTRAINT chk_storage_area_latitude_range
        CHECK (latitude IS NULL OR latitude BETWEEN -90 AND 90),
    CONSTRAINT chk_storage_area_longitude_range
...
CREATE TABLE trap (
    ...
    CONSTRAINT chk_trap_latitude_range  CHECK (latitude  BETWEEN -90 AND 90),
    CONSTRAINT chk_trap_longitude_range CHECK (longitude BETWEEN -180 AND 180)
);
...
CREATE TABLE bait_station (
    ...
    UNIQUE (group_id, code),
    CONSTRAINT chk_bait_station_latitude_range  CHECK (latitude  BETWEEN -90 AND 90),
    CONSTRAINT chk_bait_station_longitude_range CHECK (longitude BETWEEN -180 AND 180)
);
...
CREATE TABLE group_operational_area_vertex (
    ...
    CONSTRAINT chk_group_area_vertex_latitude_range  CHECK (latitude  BETWEEN -90 AND 90),
    CONSTRAINT chk_group_area_vertex_longitude_range CHECK (longitude BETWEEN -180 AND 180)
);
...
CREATE TABLE line_path_vertex (
    ...
    CONSTRAINT chk_line_path_vertex_latitude_range  CHECK (latitude  BETWEEN -90 AND 90),
    CONSTRAINT chk_line_path_vertex_longitude_range CHECK (longitude BETWEEN -180 AND 180)
);
...
CREATE TABLE nz_species (
    ...
    CONSTRAINT chk_nz_species_min_latitude_range
        CHECK (min_latitude  IS NULL OR min_latitude  BETWEEN -90 AND 90),
    CONSTRAINT chk_nz_species_max_latitude_range
        CHECK (max_latitude  IS NULL OR max_latitude  BETWEEN -90 AND 90),
    CONSTRAINT chk_nz_species_min_longitude_range
        CHECK (min_longitude IS NULL OR min_longitude BETWEEN -180 AND 180),
    CONSTRAINT chk_nz_species_max_longitude_range
        CHECK (max_longitude IS NULL OR max_longitude BETWEEN -180 AND 180),
    CONSTRAINT chk_nz_species_latitude_order
        CHECK (
            min_latitude IS NULL OR max_latitude IS NULL OR min_latitude <= max_latitude
        ),
    CONSTRAINT chk_nz_species_longitude_order
        CHECK (
            min_longitude IS NULL OR max_longitude IS NULL OR min_longitude <= max_longitude
        )
);
...
-- Keep denormalised line_sponsorship.donor_profile_id aligned with donation.donor_profile_id.
CREATE OR REPLACE FUNCTION fn_sync_line_sponsorship_donor_profile_id()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
...
CREATE TRIGGER trg_line_sponsorship_donor_profile_id_sync
    BEFORE INSERT OR UPDATE OF donation_id ON line_sponsorship
    FOR EACH ROW EXECUTE FUNCTION fn_sync_line_sponsorship_donor_profile_id();

-- Enforce minimum 3 vertices for any persisted operational area polygon.
-- Deferred constraint trigger allows multi-row edits within a transaction.
CREATE OR REPLACE FUNCTION fn_enforce_group_area_min_vertices()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
    v_area_id INT;
    v_count   INT;
...
CREATE CONSTRAINT TRIGGER trg_group_area_min_vertices
    AFTER INSERT OR UPDATE OR DELETE ON group_operational_area_vertex
    DEFERRABLE INITIALLY DEFERRED
    FOR EACH ROW EXECUTE FUNCTION fn_enforce_group_area_min_vertices();
